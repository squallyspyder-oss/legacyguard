...
export type ExecutorValidations = {
  hasToken: boolean;
  prExists: boolean;
  approved: boolean;
  checksPass: boolean;
...
export type ExecutorOutput = {
  role: 'executor';
  action: 'merged' | 'aborted' | 'pending';
  merged: boolean;
  prNumber: number;
  prUrl: string;
  mergeCommitSha?: string;
  validations: ExecutorValidations;
  reason: string;
  risks: string[];
  rollbackPlan: string;
  postMergeActions: string[];
};

export async function runExecutor(task: ExecutorInput): Promise<ExecutorOutput> {
  // Validações iniciais
  const validations: ExecutorValidations = {
    hasToken: Boolean(task.token),
    prExists: false,
    approved: task.reviewerApproved ?? false,
    checksPass: false,
    noConflicts: false,
    sandboxPassed: task.sandboxPassed ?? true,
  };
  ...
  const risks: string[] = [];
  if (!validations.approved) risks.push('PR não aprovada por reviewer');
  if (!validations.checksPass) risks.push('CI/CD checks não passaram');
  if (!validations.noConflicts) risks.push('Existem conflitos com a branch base');
  if (!validations.sandboxPassed) risks.push('Sandbox não validou as mudanças');
  ...
  try {
    ...
    await logEvent({
      ...
      metadata: { 
        ...
        repo, 
        sha: mergeCommitSha,
        twinId: task.twinContext?.twinId,
      },
    }).catch(() => undefined);
    ...
    await logEvent({
      ...
      severity: 'error',
      message: `Merge falhou: ${err?.message || err}`,
    ...
    return {
      ...
      prUrl: prData.html_url,
      validations,
      reason: `Merge falhou: ${err?.message || err}`,
...
function buildMergeCommitMessage(task: ExecutorInput, prData: any): string {
